...
import { HeartStraight, PaperPlaneTilt, RocketLaunch, CalendarDots, CalendarPlus, Question } from '@phosphor-icons/react'
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { addTravelerWish } from '@/lib/actions/traveler-wishlists'

// ─── Period options ──────────────────────────────────────────────────────────

const PERIOD_OPTIONS = [
  { value: 'imminent', label: 'Projet imminent', icon: RocketLaunch, color: '#DD9371' },
  { value: 'next_year', label: "L'année prochaine", icon: CalendarDots, color: '#0FB6BC' },
...
interface AddWishDialogProps {
  ...
  onWishAdded?: (countryCode: string) => void
}
...
export function AddWishDialog({
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await addTravelerWish(participantId, countryCode, note || undefined, desiredPeriod)
      if (result.error) {
      ...
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-500">
            Où rêvez-vous de voyager ? Ajoutez une destination à votre carte du monde.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Ce qui vous fait rêver de cette destination..."
